...
class MiddleManager
  ...
  def install_jdbc_driver
    ...
    puts x("wget https://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-java-5.1.45.tar.gz")
    puts x("tar -xzf mysql-connector-java-5.1.45.tar.gz")

    puts "Moving and renaming JDBC driver"
    puts x("mkdir /usr/share/java")
    puts x("mv mysql-connector-java-5.1.45/mysql-connector-java-5.1.45-bin.jar /usr/share/java/mysql-connector-java.jar")
    puts x("rm -rf mysql-connector-java-5.1.45*")
  end
  ...
  def install_cloudera_manager
    puts x("yum install -y yum-utils")
    puts x("yum-config-manager --add-repo https://archive.cloudera.com/cm5/redhat/7/x86_64/cm/cloudera-manager.repo")
    puts "Installing cloudera-manager-daemons"
    puts x("yum install -y cloudera-manager-daemons")
    puts x("yum install -y cloudera-manager-server")

    puts "Verifying Cloudera Manager databases are configured properly"
    puts x("/usr/share/cmf/schema/scm_prepare_database.sh mysql cmserver cmserver_user #{@conf.mysql_cm_dbs_password}")

    puts "Starting cloudera-scm-server"
    puts x("systemctl start cloudera-scm-server")
  end
